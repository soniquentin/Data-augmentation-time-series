...
def calc_length(data) -> int :
    return (len(data.columns) - 1)


def calc_nb(data) -> (int,int,dict) :
    #Labels et leur compte :
    unique_labels = np.sort( data[0].unique() )
    label_dict = {}
    for label in unique_labels :
...
def calc_IR_ID(data) -> (float,float) :
    ...
    if len(unique_labels) == 2 :
        im_rat = min(classes_count)/max(classes_count)
        im_deg = None
